...
function getParentPopup (el) {
  ...
  whileParent(el, parent => {
    if (parent !== el && isPopupElement(parent)) {
      popupEl = parent
      return false
    }
  })
...
function addToActivePopups (el) {
  const idx = activePopups.indexOf(el)
  if (!activePopups.length || idx !== activePopups.length - 1) {
    if (idx >= 0) activePopups.splice(idx, 1)
    activePopups.push(el)
  }
}

function removeFromActivePopups (el) {
  const idx = activePopups.indexOf(el)
  if (idx >= 0) activePopups.splice(idx, 1)
}
...
function setPopupState (el, action, event, skipEmitEvent) {
  ...
  if (popupState !== undefined) {
    if (skipEmitEvent || emitEvent(el, popupState === 'popup' ? POPUP_OPEN_EVENT : POPUP_CLOSE_EVENT) !== false) {
      ...
      popupState === 'popup' ? addToActivePopups(el) : removeFromActivePopups(el)
...
export function init (options) {
  ...
  document.addEventListener(eventType, event => {
    ...
    targetIsMask ? deactiveChildren(popupEl) : deactiveSiblings(getParentPopup(target), popupEl)
