...
export function show (target) {
  ...
  if (el) setPopupState(el, 'open')
}

export function hide (target, force) {
  const el = queryTarget(target)
  if (el && el.getAttribute('sparrow-popup') === 'popup') {
    if (force) el.setAttribute(POPUP_ATTR, '')
    else setPopupState(el, 'close')
  }
}
