...
const DEFAULT_OPTIONS = {
  tapStartEventType: (!isTouchDevice || ('onmousedown' in window)) ? 'mousedown' : 'touchstart',
  eventType: (!isTouchDevice || ('onclick' in window)) ? 'click' : 'tap'
}

let registered = false
let globalRegistration = document.__sparrow_popup
if (globalRegistration) registered = true
else globalRegistration = document.__sparrow_popup = {}
const activePopups = globalRegistration.activePopups = globalRegistration.activePopups || []

let lastEventType, lastTapStartEventType
...
function tapEventHandler (event) {
  ...
  if (popupEl && (lastEventType === lastTapStartEventType || event.type !== lastTapStartEventType)) {
    setPopupState(popupEl, action, event)
  }
}
...
function registerTapEventListener (eventType, tapStartEventType) {
  eventType = eventType || DEFAULT_OPTIONS.eventType
  tapStartEventType = tapStartEventType || DEFAULT_OPTIONS.tapStartEventType
  if (lastTapStartEventType !== tapStartEventType) {
    if (lastTapStartEventType) document.removeEventListener(lastTapStartEventType, tapEventHandler)
    lastTapStartEventType = tapStartEventType
    document.addEventListener(lastTapStartEventType, tapEventHandler)
  }
  if (lastEventType !== eventType) {
    if (lastEventType) document.removeEventListener(lastEventType, tapEventHandler)
    lastEventType = eventType
    document.addEventListener(eventType, tapEventHandler)
  }
}
...
if (!registered) {
  ...
  initTimer = setTimeout(registerTapEventListener, 500)
}
...
export function set (options = {}) {
  if (registered) {
    console.warn('There are more than 1 SparrowPopup instances loaded, can only call "set" method at the first one.')
    return
  }
  if (options.eventType || options.tapStartEventType) {
    if (initTimer) {
      clearTimeout(initTimer)
      initTimer = undefined
    }
    registerTapEventListener(options.eventType, options.tapStartEventType)
  }
}
