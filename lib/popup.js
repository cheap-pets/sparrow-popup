...
function queryActivePopup () {
  return document.querySelector('.dropdown.active,.popup.active,.active[popup],.modal-mask.active')
}
...
document.addEventListener(clickEvent, function (event) {
  ...
  // 查找层级
  let level = 0
  while (el && el !== document && el !== document.body && el.parentNode !== document) { // && level < 5
  ...
  if (popup) {
    ...
    if (event) dispatchCustomEvent(popup, event, false, false)
  }
})
